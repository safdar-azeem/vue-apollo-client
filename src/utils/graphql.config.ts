...
export const graphqlConfig = ({
					...
					return fromPromise(
						refreshToken()
							...
							.catch((error) => {
								...
								removeToken(tokenKey)
								// Default cleanup: clear every Apollo client store so cached
								// user data is dropped on session invalidation. The user's
								// onLogout still runs after, so they can do navigation,
								// analytics, etc. against an already-clean state.
								clearAllStores()
	...
	// Default logout cleanup: drop every Apollo store so cached user
	// data doesn't survive a session invalidation. The function is
	// defined here (not at the top of `graphqlConfig`) so that
	// `clearAllStores` is callable from the error link above — closure
	// resolution happens at call time, when `clients` is already set.
